...
{
    [Description("销售出库审核插件")]
    public class SaleOutAudit : AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject billObj in e.DataEntitys)
            {
                ...
                try
                {
                    ...
                    DynamicObject cust
                        = billObj["CustomerID"] as DynamicObject;
                    if (cust != null)
                    {
                        string custName = cust["Name"].ToString();

                        //如果根据客户名称能匹配到组织，代表出库单为内部交易单据，不需要传递到CRM
                        if (!string.IsNullOrWhiteSpace(SqlHelper.GetOrgNumberByCustName(this.Context,custName)))
                        {
                            continue;
                        }
                    }
                    ...
                    if (org != null)
                    {
                        ...
                        if (org["Number"].ToString() == "1002"|| org["Number"].ToString() == "1001"|| org["Number"].ToString() == "1012")
                        {
                            continue;
                        }
                    }
                    ...
                    #region 调用关闭接口
                    string zohoBillNo = "";
                    bool close = SqlHelper.SaleOrderIsClose(
                        this.Context,
                        billObj["Id"].ToString(),
                        out zohoBillNo);
                    if (close)
