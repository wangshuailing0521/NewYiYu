...
{
    [Description("直接调拨反审核插件")]
    public class StkTransUnAudit: AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject billObj in e.DataEntitys)
            {
                ...
                sb.AppendLine($@"接口名称：直接调拨订单取消关闭API");
                try
                {
                    #region 调用关闭接口
                    string zohoBillNo = "";
                    bool open = SqlHelper.TransApplyNoClose(
                        this.Context,
                        billObj["Id"].ToString(),
                        out zohoBillNo);
                    if (open)
                    {
                        url = "https://zoho.onetrum.com/public/index.php/api/index/open_order_from_kd";
